 ...
 * 
 * API key path is included directly into the beginning of URL path of given
 * base URL.
 */
public class ApiKeyMangler implements URLMangler {

    /** Header key for API key value. */
    private final static String API_KEY_HEADER = "fmi-apikey";
    /**
     * URL parameter key for API key value. Notice, URL parameter is not checked
     * if this key is {null} or empty. Normally, {API_KEY_HEADER} may also be
     * set for this value.
     */
    private final static String API_KEY_PARAMETER = null;
    /** URL path separator. */
    private final static String URL_SEPARATOR = "/";
    /** URL path for API key. */
    private final static String API_KEY_URL_PATH = URL_SEPARATOR
            + API_KEY_HEADER + URL_SEPARATOR;

    /**
     * @param {HttpServletRequest} request May not be {null}.
     * @return {String} API key if given in request header or as request
     *         parameter.
     */
    private String getApiKey(final HttpServletRequest request) {
        ...
        if (null != API_KEY_PARAMETER && !API_KEY_PARAMETER.isEmpty()
                && (null == apiKey || apiKey.isEmpty())) {
            // API key was not in header.
            // Try to get it from parameters.
            final String[] paramApiKey = request
                    .getParameterValues(API_KEY_PARAMETER);
            if (null != paramApiKey && 0 < paramApiKey.length
                    && null != paramApiKey[0] && !paramApiKey[0].isEmpty()) {
    ...
    /**
     * Get API key from the request header or from URL parameters and append API
     * key as path to the base URL.
     * 
     * Also, see super URLMangler class for function description.
     */
    public void mangleURL(StringBuilder baseURL, StringBuilder path,
            Map<String, String> kvp, URLType type) {
        try {
            // Change backlinks of GeoServer URLs.
            if (null != baseURL && URLType.EXTERNAL != type) {
                // Use context holder to get the request object.
                final RequestAttributes attributes = RequestContextHolder
                        .currentRequestAttributes();
                // API key is related to the HTTP requests.
                // So, make sure HTTP request is at hand.
                if (attributes instanceof ServletRequestAttributes) {
                    final HttpServletRequest request = ((ServletRequestAttributes) attributes)
                            .getRequest();
                    if (null != request) {
                        ...
                        if (null != apiKey && !apiKey.isEmpty()) {
                            ...
                            if (0 < baseURL.length()) {
                                ...
                                if (!baseUrlPath.isEmpty()) {
                                    if (!baseUrlPath.startsWith(apiKeyPath
                                            + URL_SEPARATOR)) {
                                        final int pathIndex = baseURL
                                                .indexOf(baseUrlPath);
